...
#endif
  PrecisionType* parametersToReturn = (PrecisionType*) malloc( this->numberOfParameters * sizeof( PrecisionType ) );
  memcpy( parametersToReturn, this->controlPointGridImage->data, this->numberOfParameters * sizeof( PrecisionType ) );
  return (BSplineTransformation::PrecisionType*) parametersToReturn;
}
...
void BSplineTransformation::InitialiseLevel( unsigned int levelIn )
{
  ...
  {
    supremo_print_warning( "BSplineTransformation::InitialiseLevel()" );
    supremo_print_warning( "Level was already initialised. Neither refining grid, nor updating bending-energy weight." );
    return;
  }
  ...
  {
    supremo_print_error( "Initialisation only implemented to go from one level to the next." );
...
nifti_image* BSplineTransformation::GetDeformationVectorField( nifti_image* targetImageIn )
{
  ...
  {
    supremo_print_error("BSplineTransformation::GetDeformationVectorField()");
    supremo_print_error( "The spline control point grid includes affine transformations." );
    supremo_print_error( "This function cannot be used when the control point grid includes affine transformations" );
    supremo_print_error( "as it assumes the axes of the control point grid and dense vector field are aligned." );
  ...
  {
    supremo_print_error( "reg_spline_CPGToDVF()" );
    supremo_print_error( "The spline control point grid and the dense vector field must have the same number of components for each vector" );
    supremo_print_error( "and these must be either 2 (for a 2D transformation) or 3 (for a 3D transformation)" );
    supremo_exit( 1, __FILE__, __LINE__ );
  }
...
void BSplineTransformation::CPGToDVF2D( nifti_image *denseVectorFieldImage )
{
  ...
  nifti_image* cpgImage = this->controlPointGridImage;

  //use openmp if available and not debug build
#if defined (NDEBUG) && defined (_OPENMP)
#pragma omp parallel for default(none) \
	shared(cpgImage, denseVectorFieldImage, DVFSpacingInCPG, \
  ...
  for ( yDVF = 0; yDVF < denseVectorFieldImage->ny; yDVF++ )
  {
    ...
    if ( firstYCPG < 0 || firstYCPG + 4 > cpgImage->ny )
    {
    ...
    else
    {
      ...
      tmpXValsX = new PrecisionType[cpgImage->nx];
      tmpXValsY = new PrecisionType[cpgImage->nx];

      //loop over x in CPG
      for ( xCPG = 0; xCPG < cpgImage->nx; xCPG++ )
      {
        //sum the 4 CPs in y direction that affect yDVF
        valX = valY = 0.0;
        for ( yCPG = 0; yCPG < 4; yCPG++ )
        {
          indCPG = xCPG + (yCPG + firstYCPG) * cpgImage->nx;
      ...
      for ( xDVF = 0; xDVF < denseVectorFieldImage->nx; xDVF++ )
      {
        ...
        if ( firstXCPG < 0 || firstXCPG + 4 > cpgImage->nx )
        {
...
#pragma omp parallel for default(none) \
  ...
  for ( zDVF = 0; zDVF < denseVectorFieldImage->nz; zDVF++ )
  {
    ...
    if ( firstZCPG < 0 || firstZCPG + 4 > cpgImage->nz )
    {
    ...
    else
    {
      ...
      //initialise variables to store intermediate results
      tmpXYValsX = new PrecisionType[cpgImage->nx * cpgImage->ny];
      tmpXYValsY = new PrecisionType[cpgImage->nx * cpgImage->ny];
      tmpXYValsZ = new PrecisionType[cpgImage->nx * cpgImage->ny];

      //loop over y in CPG
      for ( yCPG = 0; yCPG < cpgImage->ny; yCPG++ )
      {
        //loop over x in CPG
        for ( xCPG = 0; xCPG < cpgImage->nx; xCPG++ )
        {
          valX = valY = valZ = 0.0;
          indTmp = xCPG + yCPG * cpgImage->nx;
          //sum the 4 CPs in z direction that affect zDVF
          for ( zCPG = 0; zCPG < 4; zCPG++ )
          {
            indCPG = indTmp + (zCPG + firstZCPG) * cpgImage->nx * cpgImage->ny;
      ...
      for ( yDVF = 0; yDVF < denseVectorFieldImage->ny; yDVF++ )
      {
        ...
        else
        {
          ...
          tmpXValsX = new PrecisionType[cpgImage->nx];
          tmpXValsY = new PrecisionType[cpgImage->nx];
          tmpXValsZ = new PrecisionType[cpgImage->nx];
          ...
          for ( xCPG = 0; xCPG < cpgImage->nx; xCPG++ )
          {
            ...
            for ( yCPG = 0; yCPG < 4; yCPG++ )
            {
              indTmp = xCPG + (yCPG + firstYCPG) * cpgImage->nx;
...
#if defined (NDEBUG) && defined (_OPENMP)
#pragma omp parallel for default(none) \
	shared(controlPointGridImage, CPGPtrX) \
	private(i)
#endif
  for (i = 0; i < controlPointGridImage->nvox; ++i)
  {
    CPGPtrX[i] = (PrecisionType) 0.;
  }
                                        ...
                                        bool normaliseKernel )
{
  ...
  // initialise CPG to 0 by setting it explicity, not multiplying. Multiplication can 
  // go wrong in case CPG contains NaNs
  int i;
...
BSplineTransformation::PrecisionType* BSplineTransformation::GetDVFGradientWRTTransformationParameters( nifti_image* denseDVFIn )
{
  ...
  outDVFGradWRTTrafoParams->data = calloc( outDVFGradWRTTrafoParams->nvox, outDVFGradWRTTrafoParams->nbyper );
  ...
  {
    supremo_print_error( "Error in BSplineTransformation::GetDVFGradientWRTTransformationParameters()" );
    supremo_print_error( "The spline control point grid and the dense vector field must have the same number of components for each vector" );
    supremo_print_error( "and these must be either 2 (for a 2D transformation) or 3 (for a 3D transformation)" );
    supremo_exit( 1, __FILE__, __LINE__ );
  }
...
{
  // Allocate the return vector holding the single image defining this transformation, i.e. the control point grid
  std::vector<nifti_image*> tranformImages;
  tranformImages.push_back( this->controlPointGridImage );
  
  return tranformImages;
}
