...
// Tolerances allowed 
// For the update we accummulate errors need to be more relaxed with the threshold
constexpr auto EPS_SINGLE = 0.05;
...
int main( int argc, char **argv )
{
  ...
  // This test is similar to the Super-resolution restart one, but does the first reconstruction with a zero-correspondence model. Hence the 
  // loaded RCM file will only be used once the iniital reconstruction was performed. 
  ...
  // Decide to do an update of the reconstruction during object-construction
  auto moCoReco = std::make_shared<MoCoReconSuperResolutionIBP>( mcrIterations, true );
  ...
  // Pretend the optimisation was performed and the correspondence model was updated
  // Feed the input data into the correspondence model
  // but before check that the data is of expected type
  if (rcmImage->nbyper != sizeof( CorrespondenceModel::PrecisionType ))
  {
