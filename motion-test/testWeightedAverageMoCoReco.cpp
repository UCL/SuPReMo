...
int main( int argc, char **argv )
{
  ...
  const unsigned int numberOfLevelsToPerform = (unsigned int)parser->getCmdOptionAsInt( "-ln" );
  ...
  auto bsplTrafo = std::make_shared<BSplineTransformation>( referenceStateImage, numberOfLevelsToPerform, bSplineCPGSpacing );
  ...
  for (unsigned int i = 0; i < numberOfLevelsToPerform; ++i)
  {
    correspModel->InitialiseLevel( i );
  }
  ...
  // image acquisition
  std::shared_ptr<ImageAcquisition> noAcquisition = std::make_shared<NoImageAcquisition>();

  // Motion-compensated image reconstruction to test
  auto weightedAvgMoCoReco = std::make_shared<MoCoReconWeightedAverage>();
  weightedAvgMoCoReco->SetCorrespondenceModel( correspModel );
  weightedAvgMoCoReco->SetReconstructionGeometryImage( referenceStateImage );
  weightedAvgMoCoReco->SetSurrogateSignals( surrogateSignalsVec );
  weightedAvgMoCoReco->SetDynamicImages( dynamicImages );
  weightedAvgMoCoReco->SetImageAcquisition( noAcquisition );
  weightedAvgMoCoReco->Update();

  // Save the reconstructed image if requried
  if (parser->cmdOptionExists( "-outMCR" ))
  {
    reg_io_WriteImageFile( weightedAvgMoCoReco->GetReconstructedImage(), parser->getCmdOptionAsString( "-outMCR" ).c_str() );
  }

  // Compare the output with the expected outcome
  return checkImageEquality<float>( weightedAvgMoCoReco->GetReconstructedImage(), mcrImage, EPS_SINGLE );  
}
